...
Yellow bullets: 23 → 12. Merged: truthdb #87–#112, docs #41–#67.
...
COLMETADATA precedes the first row). **Stage 9's sp_prepare handle family
is DONE (#112)** — and its `catalog_version`/rebind design was MOOT ON
...
(sp_describe_first_result_set; the exit matrix, whose JDBC leg needs
...
- **Prepared statements work (Stage 9).** *(Updated by #112.)* `sp_executesql` and the whole `sp_prepare`/`sp_execute`/`sp_prepexec`/`sp_unprepare` handle family work, with the handle returned via RETURNVALUE (pytds drives the family live in CI). There is deliberately no plan cache and no rebind machinery — every execute re-parses and re-binds against the live catalog, so DDL between prepare and execute is recompile-on-schema-change by construction (the plan's `catalog_version` design was moot on audit). Remaining: DONEPROC/DONEINPROC/RETURNSTATUS framing and multi-RPC requests (JDBC's default flow needs both — named prerequisites of the exit matrix), and `sp_describe_first_result_set`.
...
- TDS Attention (cancel) now aborts a running statement (#86); `ALL_HEADERS` is **validated** since #88. No `sp_describe_first_result_set` (Stage 9). The idle/txn reaper shipped in #89; the executor stopped materializing a single-table bare-column SELECT's input in #101, but the reply is still a whole `BatchOutcome` — and "bounded sinks" is not how that gets closed (Stage 6, see **(c)**). No DDL version counter (relocated to Stage 9, with its prepared-statement consumer).
