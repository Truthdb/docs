...
Yellow bullets: 23 → 15. Merged: truthdb #87–#105, docs #41–#60.

**Stage 6's Engine actor bullet is CLOSED, and Stage 7's covering-seek gap
followed (#106):** `CREATE INDEX ... INCLUDE` stores original values in the
leaves, and covering seeks answer without a Key Lookup — the design change
(value-carrying leaves) the gap note called for, since the one-way sort-key
leaves could never serve. The planner bullet's one remaining gap is
**row-count tie-breaks**, still impossible as written: no statistics facility
exists anywhere, so the next piece of work there is a persisted row count
maintained by DML (a counter page + WAL record, or a catalog rewrite per DML)
— its own design, not the bullet as written.
