...
**Stage 6's Engine actor bullet is CLOSED.** The front of the queue moves to
Stage 7's **planner** bullet, whose two remaining gaps are recorded there as
impossible *as this plan writes them* (a covering-index seek over one-way
collation sort keys; row-count tie-breaks with no statistics facility of any
kind). The next piece of work is choosing and building the design change that
makes one of them possible — value-carrying (`INCLUDE`) index leaves, or a
persisted row count maintained by DML — not building the bullet as written.
...
- Planner: ~~no projection pruning~~ **pruning added (#102)** — a scan decodes only the columns the query reads (projection ∪ WHERE), 4.9x on a narrow projection over a wide table; no row-count tie-breaks (**no statistics facility exists at all**); `KeyLookup` folded into `IndexSeek` (**a covering seek is impossible for CI/AI/default-collation strings — #94's index leaves hold one-way sort keys and no values**); ~~UNIQUE only via `CREATE UNIQUE INDEX`~~ **inline `UNIQUE` constraint added (#77)** (Stage 7).
