...
- **Query engine hashes and spills (Stage 8).** *(Updated by PRs #71, #78–#80.)* Hash join (equijoins), hash aggregate, and hash DISTINCT replaced the old naive/O(n²) operators (#71). Spill is now built (#78–#80): a temp-extent `RowSpool` (self-describing cap-free row codec) backs an **external merge sort** for `ORDER BY`, **grace-hash aggregation** for `GROUP BY`, and **grace-hash INNER join**, each spilling past a per-query memory budget instead of erroring or doubling memory. Remaining: grace-hash for **LEFT/RIGHT/FULL** joins (in-memory for now), DISTINCT grace-hash, and **streaming scans** so the *input* materialization is bounded too (today the operators bound their own working set but `build_source` still materializes the input; the sort/agg/join tests force-spill via a tiny test budget).
