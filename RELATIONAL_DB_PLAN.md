...
### Follow-up session (2026-07-14, PRs #71–#81)
...
5. **Adversarial review before merge** for the three correctness-critical PRs. It found and blocked, pre-merge: the Hash/Eq f64 bug (#71); three HIGH row-lock isolation holes — an IX→S `combine` dirty-read, a char-key-vs-numeric-literal under-lock, and a secondary-UNIQUE-index not serialized (#72); and an ambiguous-inner-column rebind (#73). All fixed and re-verified.

Forward coupling to watch: when Stage 5 makes WHERE equality collation-aware, a case-insensitive character **key** column would then match rows the (binary) row-lock key does not cover — so such columns must be excluded from point row-locking (extend `literal_pins_key` / `key_columns_row_lockable`).
