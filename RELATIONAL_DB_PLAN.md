...
Yellow bullets: 23 → 12. Merged: truthdb #87–#113, docs #41–#68.
...
bounded-sinks class of finding). The remaining yellows are Stage 9's tail
(the exit matrix, whose JDBC leg needs
DONEPROC/DONEINPROC framing and multi-RPC request support — both named in
the bullet's gap note), Stage 10 (ALTER TABLE ADD column — needs a
version-tolerant row decode), Stage 11 (SELECT-list/HAVING/derived-table
correlation; EXEC sp_executesql text path), and Stage 12 (sharded
latches/partitioned lock manager; heartbeats-during-scan test).
...
- 🟡 Row locks (`LockKey::Row`) + IX intents, escalation at ~5000 locks; waits-for-graph deadlock detector (200 ms, youngest victim, error 1205); key-range locks on B+ leaves for true SERIALIZABLE. *(done #72: `Resource::Row(object_id, xxh64(key bytes))` + IX intents; point INSERT/UPDATE/DELETE take row locks, so writers to distinct rows run concurrently — with documented fallbacks to table locks for secondary-UNIQUE-index tables, FK parents, REAL/FLOAT keys, and character keys compared to non-string literals (`S.combine(IX)` → `X`, as there is no SIX mode). So "still table-granular" is stale. Gaps: **key-range locks unbuilt** (SERIALIZABLE phantom protection rests on table-granular S locks — deferred deliberately: a next-key lock on every insert hot-path to speed a rare isolation level is net-negative); **escalation exists but at 1000 keys, not ~5000, and has no test of its own**; and deadlock **detection is synchronous-at-park plus a 5 s timeout backstop, not a 200 ms waits-for-graph poll**, so a waiter can sit up to 5 s behind a long-running holder.)*
...
- **Prepared statements work (Stage 9).** *(Updated by #112.)* `sp_executesql` and the whole `sp_prepare`/`sp_execute`/`sp_prepexec`/`sp_unprepare` handle family work, with the handle returned via RETURNVALUE (pytds drives the family live in CI). There is deliberately no plan cache and no rebind machinery — every execute re-parses and re-binds against the live catalog, so DDL between prepare and execute is recompile-on-schema-change by construction (the plan's `catalog_version` design was moot on audit). Remaining: DONEPROC/DONEINPROC/RETURNSTATUS framing and multi-RPC requests (JDBC's default flow needs both — named prerequisites of the exit matrix). `sp_describe_first_result_set` is done too (#113): first-result-set contract, statically-derivable shapes only, 11514 otherwise.
